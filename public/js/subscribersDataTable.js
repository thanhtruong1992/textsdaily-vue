$(document).ready(function() {
	var pre = 10;
	var arrField = [ "check", "id", "phone", "first_name","last_name", "status"];
	...
	var arrColumn = [
	{
		...
		data: "status", mRender: function(data, type, row) {
			return data == "SUBSCRIBED" ? "Active" : "Inactive";
		}
	}];
	$(".custom-field-table").each(function() {
		arrColumn.push({
			data : $(this).attr("id")
		});
		arrField.push($(this).attr("id"));
	});

	var arrHiddenField = [];
	for (var i = 6; i < arrColumn.length; i++) {
		arrHiddenField.push(i);
	}
