...
function selec2Validate(ele) {
	...
	if ($(ele).hasClass('error')) {
		...
		$('.select2-selection--multiple', parEle).addClass('error');
	} else {
		$('label.error', parEle).remove();
		$('.select2-selection--multiple', parEle).removeClass('error');
	}
}
...
jQuery(function($) {
	...
	// Get all option value select when form loaded
	var subscriber_list_option = [];
	$('#selectList > option:selected').each(function() {
		subscriber_list_option.push(this.value);
	});
	...
	$('.select-sender').on('select2:select', function (e) {
	    ...
	    if (sender_list_option.length >= 0 && $.inArray( data.id, sender_list_option) == -1) {
	    		// call ajax update
	        	$.ajax({
	    			type: "POST",
	            ...
	            success: function(success) {
	                if ( success["status"] ) {
	                		...
	                		$.each(sender_list_option, function(index, value){
    ...
    $('#addShortLink').click(function() {
        ...
        var campaing_id = $("input[name='campaign_id']").val();
        ...
        if(!flag){
        	...
        	if(!!checkDateTime) {
            	...
            	if(isSubmit == "false") {
            		...
            		$.fn.summaryCampaign(data, function(res) {
            			// success
	...
	$('#reset-campaign').on('click', function() {
		...
		$('#selectList > option').each(function() {
			if ($.inArray(this.value, subscriber_list_option) != -1) {
	...
	$('#update-campaign').on('click', function() {
	...
	$('.button-send').on('click',function() {
		...
		$('input[name="test_recipients"]').rules("add", {
			required : true,
			messages : {
				required : window.link.test_recipients_error_message
			}
		});
			...
			$.ajax({
				...
				success : function(success) {
					if (success["status"]) {
						...
						showErrorModalPopup(false, [], [], [], [], [], success["message"]);
					}
	...
	function showErrorModalPopup(status, numberSuccess, numberInvalid, numberPending, numberReject, numberExpired, message = "") {
		...
		if (numberSuccess.length <= 0) {
			...
			$.each(numberSuccess, function(index, item) {
				...
				$('#itemSuccessGroup').append(
						"<span class='item-phone-number-success'>" + item
								+ "</span>");
			})
		}
		...
		if (numberPending.length <= 0) {
			...
			$('#messagePeddingSuccess').hide();
			$('#messagePeddingFail').show();
		} else {
			$('#contentSMSPending').show();
			$("#messagePeddingFail").hide();
			$('#messagePeddingSuccess').show();
		...
		if (numberExpired.length <= 0) {
			...
			$.each(numberExpired, function(index, item) {
				item = index > 0 ? "; " + item : item;
				$('#itemExpiredGroup').append(
						"<span class='item-phone-number-expired'>" + item
								+ "</span>");
			})
		}
		...
		if (numberInvalid.length <= 0) {
			...
			$.each(numberInvalid, function(index, item) {
				item = index > 0 ? "; " + item : item;
				$('#itemFailedGroup').append(
						"<span class='item-phone-number-invalid'>" + item
								+ "</span>");
			})
		}
    ...
    $('#loadPersonalize').on('click', function () {
    		if ($('.select-multiple').valid()) {
    			$('#personalizeModal').modal({"backdrop" : "static", "show": true});
	        ...
	        columns: [
	    		...
	    		{ data: "", mRender: function (data, type, row) {
	    			...
	    			return '<label class="custom-control custom-checkbox">' +
	                		'<input type="checkbox" class="custom-control-input frmModalCheck" value="' + field_name + '" name="Mark"/>' +
	...
	$("#unsubscribe").on('click', function() {
		$.ajax({
			type: "GET",
			url: window.link.api_add_ubsubscriber,
			data: {
			},
			success: function(res) {
				var message = $("#message").val();
				var position = $('#message').getCursorPosition();
				message = message.substring(0, position) + res.link + message.substring(position, message.length);
				$("#message").val(message);
				$("#message").trigger('onkeyup');
				$.fn.showNotification("success", {content: res.message});
			},
			error: function(err) {
				$.fn.showNotification("error", {content: err.messaeg});
			}
		});
	});
});
