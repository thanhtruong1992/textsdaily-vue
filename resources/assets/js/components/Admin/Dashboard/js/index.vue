...
export default {
  ...
  mounted() {
    ...
    $(".select-filter").on("change", function(e) {
      ...
      switch (value) {
        case "month":
          !$("#select-date").hasClass("invisible")
            ? $("#select-date").addClass("invisible")
            : "";
          !$("#select-month").hasClass("invisible")
            ? $("#select-month").addClass("invisible")
            : "";
          break;
        case "day":
          !$("#select-date").hasClass("invisible")
    ...
    function addOrRemoveOption(totalDate, currentDate) {
      ...
      if (totalOption > totalDate) {
        for (var i = totalDate + 1; i <= totalOption; i++) {
        ...
        for (var i = totalOption + 1; i <= totalDate; i++) {
          $(".select-date").append(
            $("<option>", {
              value: i,
              text: i
      ...
      if (value.length == 0) {
        ...
        google.visualization.events.addListener(chart, "ready", function() {
          var container = document.querySelector(
            "#chartDashBoard > div:last-child"
          );
          function setPosition() {
            var tooltip = container.querySelector(
